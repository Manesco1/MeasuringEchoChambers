...
df_ind2<- subset(df_ind, select=-c(Cluster))
df_rs <- df_ind2
...
## Homogeneity of variance (t-test requirement)
```{r}
df_rs$EC_Index%>% class()
df_rs$EC_Index <- df_rs$EC_Index%>% as.numeric()
car::leveneTest(EC_Index ~ factor(ideology), df_rs)
bartlett.test(EC_Index ~ factor(ideology), df_rs)
```

# Appendix Material 

```{r}
# Show that there is not a massive difference between different measures of centrality in our dataset. 

ggpairs(dfctl, columns = 2:ncol(dfctl))

df_ind$Size <- unlist(size)

# This is for the appendix. (Make it nicer!)
Degree <- list()
Eig <- list()
Hub <- list()
Authority <- list()
...
PageRank <- list()
for (sub in totsubs_list) {
    Degree <- append(Degree, degree(eval(parse(text=paste("g_",sub,"_ud",sep="")))))
    Eig <- append(Eig, evcent(eval(parse(text=paste("g_",sub,"_ud",sep=""))))$vector)
    Hub <- append(Hub, hub.score(eval(parse(text=paste("g_",sub,"_ud",sep=""))))$vector)
    Authority <- append(Authority, authority.score(eval(parse(text=paste("g_",sub,"_ud",sep=""))))$vector)
    Closeness <-append(Closeness, closeness(eval(parse(text=paste("g_",sub,"_ud",sep="")))))
    Betweenness <- append(Degree, betweenness(eval(parse(text=paste("g_",sub,"_ud",sep="")))))
    PageRank <- append(PageRank, page_rank(eval(parse(text=paste("g_",sub,"_ud",sep=""))))$vector)
}
...
#png("IndexFigures.png", width=4000, height=2000, res=500)


#tiff("ECIbySubandIdeology.tiff", units="in", width=8, height=5, res=300)

ggplot(df_rs, aes(x = reorder(Subreddit,EC_Index),y = EC_Index, fill = Ideology)) +
...
```{r}
model <- aov(index ~ ideology + sizers, data = testdf_rs)
mk <- kruskal.test(index ~ ideology, data = testdf_rs)
model %>% summary()
mk

dunn <- dunn.test(testdf_rs$index,testdf_rs$ideology)
```
